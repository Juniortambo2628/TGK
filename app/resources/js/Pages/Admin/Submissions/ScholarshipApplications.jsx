...
export default function ScholarshipApplications({ applications = [] }) {
    ...
    const { selectedIds, toggleSelect, toggleSelectAll, handleBulkDelete, clearSelection } = useSelectableList(filteredApplications, { routeName: 'admin.scholarship-applications.destroy', confirmMessage: 'applications' });

    const handleStatusChange = (id, status) => {
        router.patch(
            route('admin.scholarship-applications.status', id),
            { status },
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleDelete = (id) => {
        ...
        router.delete(route('admin.scholarship-applications.destroy', id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleExport = () => {
        exportCSV(filteredApplications, 'scholarship-applications.csv');
    };
                            ...
                            <table className="w-full text-left text-sm">
                                <thead>
                                    <tr className="border-b border-brand-hairline bg-brand-panel/60">
                                        ...
                                        <th className="px-4 py-3 text-xs font-semibold tracking-wide text-brand-charcoal/50">Organisation</th>
                                        <th className="px-4 py-3 text-xs font-semibold tracking-wide text-brand-charcoal/50">Amount</th>
                                ...
                                <tbody className="divide-y divide-brand-hairline">
                                    {filteredApplications.map((row) => (
                                        <tr key={row.id} className="transition-colors hover:bg-brand-panel/40">
                                            ...
                                            <td className="px-4 py-3 text-brand-charcoal/70">{row.organisation || '—'}</td>
                                            <td className="px-4 py-3 text-brand-charcoal/70">
                                                {row.amount ? `KES ${Number(row.amount).toLocaleString()}` : '—'}
                                            </td>
                                            <td className="px-4 py-3">
                                    ...
                                    {filteredApplications.length === 0 && (
                                        <tr>
                                            <td colSpan={8} className="px-6 py-8 text-center text-sm text-brand-charcoal/40">
                        ...
                        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {filteredApplications.map((app) => (
                                ...
                                >
                                    ...
                                    {app.organisation && <p className="text-xs text-brand-charcoal/50">{app.organisation}</p>}
                                    {app.amount && <p className="text-xs text-brand-charcoal/70 font-bold">KES {Number(app.amount).toLocaleString()}</p>}
